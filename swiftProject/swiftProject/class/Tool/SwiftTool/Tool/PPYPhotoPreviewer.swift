...
class PPYPhotoPreviewer: UIView, UIScrollViewDelegate {
    ...
    func preview(fromImageView: UIImageView, container: UIView) {
        ...
        containerView?.mj_origin = CGPoint.zero
        containerView?.width = width // containerView 的宽度是屏幕的宽度
        
        let image: UIImage? = fromTheImageView?.image
        
        // 计算 containerView 的高度
        if (image?.size.height)! / (image?.size.height)! > self.height/width {
            containerView?.height = floor((image?.size.height)! / ((image?.size.width)! / width))
        }else {
            var height: CGFloat? = (image?.size.height)! / (image?.size.width)! * width
        ...
        scrollView?.contentSize = CGSize(width: CGFloat(width), height: CGFloat(max((containerView?.height)!, self.height)))
    ...
    @objc func doubleTap(_ recognizer: UITapGestureRecognizer) {
        if (scrollView?.zoomScale)! > CGFloat(1.0) {
            ...
            let xSize: CGFloat = width / newZoomScale
            let ySize: CGFloat = height / newZoomScale
