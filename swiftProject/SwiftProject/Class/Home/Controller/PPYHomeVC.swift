...
class PPYHomeVC: PPYBaseViewController {
    ...
    private lazy var pageTitleView: PPYPageTitleView = {
        ...
        let pageTitleView = PPYPageTitleView(frame: CGRect(x: 0, y: STATUS_NAV_BAR_Y , width: KScreenWidth, height: 41), titles: titles, config: config)
    ...
    private lazy var pageContentView: PPYPageContentView = {
        ...
        for i in 0..<titles.count {
            ...
            if i == 0 {
              ...
              childControllers.append(waterFlowLayoutVC)
            }else{
        ...
        let pageContentView = PPYPageContentView(frame: CGRect(x: 0, y: pageContentViewY, width: KScreenWidth, height: KScreenHeight - pageContentViewY), parentVC: self, childVCs: childControllers)
