...
extension UIAlertAction {
    var tag: NSInteger? {
        get {
            return objc_getAssociatedObject(self, &ycTag) as! NSInteger?
        }
        set(newValue) {
            objc_setAssociatedObject(self, &ycTag, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
}
...
extension UIAlertController {
    ...
    /*
     弹出提示框+关闭按钮（默认叫关闭）
     */
    public class func alert(title: String?, message: String?,cancel: CancelBlock? = nil) {
        ...
        // 添加控制器上的按钮
        alertVC.addAction(UIAlertAction(title:  "确定", style: .destructive, handler: { (UIAlertAction) in
