...
protocol PPYPageTitleViewDelegate: NSObjectProtocol {
    func selectedIndexInPageTitleView(pageTitleView: PPYPageTitleView, selectedIndex: Int)
}

import UIKit

class PPYPageTitleView: UIView {
    
    var config: PPYPageTitleViewConfig?
    ...
    weak var pageTitleViewDelegate: PPYPageTitleViewDelegate?
    
    override init(frame: CGRect) {
        super.init(frame: frame)
    }
    
    convenience init(frame: CGRect, titles: [String], config: PPYPageTitleViewConfig) {
        self.init()
        self.backgroundColor = UIColor.white
        self.frame = frame
        if titles.count < 1 {
            NSException(name: NSExceptionName(rawValue: "PPYPagingView"), reason: "标题数组元素不能为0", userInfo: nil).raise()
        }
        self.titles = titles
        self.config = config
        setupUI()
    ...
    private func setupButtons() {
        ...
        for title in self.titles {
            ...
            if let titleFont = self.config?.titleFont {
                let tempWidth = title.PPY_widthWithString(font: titleFont, size: CGSize(width: 0, height: 0))
        ...
        if self.allBtnWidth <= self.bounds.width {
            ...
            for (index, title) in self.titles.enumerated() {
                ...
                let tempWidth = title.PPY_widthWithString(font: (self.config?.titleFont)!, size: CGSize(width: 0, height: 0)) + (self.config?.spacingBetweenButtons)!
                if tempWidth > btnW {
            ...
            for (index, title) in self.titles.enumerated() {
                let btnW = title.PPY_widthWithString(font: (self.config?.titleFont)!, size: CGSize(width: 0, height: 0)) + (self.config?.spacingBetweenButtons)!
                let btn = UIButton(type: .custom)
                btn.frame = CGRect(x: btnX, y: btnY, width: btnW, height: btnH)
                btnX += btnW
                btn.tag = index
...
extension PPYPageTitleView {
    @objc func btnDidClick(sender:UIButton) {
        ...
        UIView.animate(withDuration: 0.1) {
            self.indicatorView.width = sender.currentTitle!.PPY_widthWithString(font: (self.config?.titleFont)!, size: CGSize(width: 0, height: 0))
...
extension PPYPageTitleView {
    ...
    private func indicatorScrollAtStatic(progress: CGFloat, originalBtn: UIButton, targetBtn: UIButton) {
        ...
        let targetBtnTextWidth: CGFloat = (targetBtn.currentTitle?.PPY_widthWithString(font: (self.config?.titleFont)!, size: CGSize.zero))!
        ...
        let originalBtnTextWidth: CGFloat = (originalBtn.currentTitle?.PPY_widthWithString(font: (self.config?.titleFont)!, size: CGSize.zero))!
...
extension String {
    ...
    func PPY_widthWithString(font: UIFont, size: CGSize) -> CGFloat {
