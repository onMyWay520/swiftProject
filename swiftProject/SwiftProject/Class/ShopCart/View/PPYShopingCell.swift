...
import UIKit
protocol PPYShoppingCellDelegate : NSObjectProtocol {
...
class PPYShopingCell: PPYBaseTableViewCell {
    ...
    //代理协议
    weak var delegate: PPYShoppingCellDelegate?
    //显示UI界面
    ...
    fileprivate lazy var selectButton: UIButton = {
        ...
        selectButton.addTarget(self, action:#selector(chooseClick), for: UIControlEvents.touchUpInside)
    ...
    ///商品旧价格标签  自定义Label
    fileprivate lazy var  oldPriceLabel: PPYPriceLabel = {
        
        let old = PPYPriceLabel()
    ...
    fileprivate lazy var subtractButton: UIButton = {
        ...
        btn.addTarget(self, action:#selector(subtractClick), for: UIControlEvents.touchUpInside)
        return btn
    }()
