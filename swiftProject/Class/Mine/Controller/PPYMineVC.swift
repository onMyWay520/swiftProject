...
import UIKit
import Kingfisher
let HeadViewHeight = KScreenHeight/3.0
class PPYMineVC: PPYBaseTableViewController {
    ...
    private lazy var headView: UIImageView = {
        let headView = UIImageView(frame: CGRect(x: 0.0, y: 0.0, width: KScreenWidth, height: HeadViewHeight))
...
//        tableView.deselectRow(at: indexPath, animated: true)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if offsety <= 0 {
            headView.frame = CGRect(x: 0.0, y: 0.0, width: KScreenWidth, height: HeadViewHeight-offsety)
        }else {
            let height = (HeadViewHeight-offsety) <= 0.0 ? 0.0 : (HeadViewHeight-offsety)
            headView.frame = CGRect(x: 0.0, y: 0.0, width: KScreenWidth, height: height)
